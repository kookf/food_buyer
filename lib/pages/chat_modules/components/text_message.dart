...
import 'package:food_buyer/pages/chat_modules/components/forward_page.dart';
...
class TextMessage extends StatelessWidget {
  ...
  /// 舉報
  requestDataWithReport()async{
    var params = {'from_user_id': message.userId, 'msg_id': message.msgId};
    var json =
    await DioManager().kkRequest(Address.chatReport, bodyParams: params);
    if (json['code'] == 200) {
      BotToast.showText(text: '舉報成功,隨後我們將會處理');
    } else {
      BotToast.showText(text: json['message']);
    }
  }
    ...
    return GestureDetector(
        onLongPressStart: (olpdt) async {
          ...
          var itemView = LongMenuItem(
            ...
            reportVoid: (){
              requestDataWithReport();
            },
            forwardVoid: (){
              Get.to(ForwardPage(message));
            },
            copyVoid: (){
